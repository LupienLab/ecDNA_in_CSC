...
id = "AA9S_T"
sample = c("AA9S_T", "G797_T", "G837_T", "G900_T") 
sample = sample[sample %ni% id]

unifiedPeak <- fread(paste0("/Users/chupan/Documents/data_su2c/scATAC/unifiedPeak/sub_malign.unifiedPeakSet.bed"), header = T, data.table = F) %>% 
...
condition <- factor(rep("all", ncol(readCount)))
colData <- DataFrame(condition)
rownames(colData) <- colnames(readCount)

dds <- DESeqDataSetFromMatrix(readCount, colData, design = ~1)

vsd <- vst(dds, blind = TRUE)
pca <- plotPCA(vsd, returnData = TRUE)
percentVar <- round(100 * attr(pca, "percentVar"))

pdf(paste0("./outputs/PCA_subMalign.pdf"), width = 3.15, height = 4)
