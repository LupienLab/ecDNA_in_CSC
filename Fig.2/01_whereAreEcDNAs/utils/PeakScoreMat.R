getPeakScoreMat <- function(
  ...
  peak <- whereAreEcDNAsObj@peak
  fragment <- whereAreEcDNAsObj@sampleColData$fragfile
  #return(as.list(environment()))
  fragment_data <- fread(fragment, header = FALSE)
  # 给列命名，第四列是细胞名称
  ...
  # 过滤 peaks
  geneRegions <- peak[ BiocGenerics::which(seqnames(peak) %bcni% excludeChr ) ]
  ...
  rownames(peakMat) <- paste0(
    seqnames(allPeakGR), ":", start(allPeakGR), "-", end(allPeakGR))
...
RunMACS2 <- function(frags = NULL,
  ...
  peak <- read.table(file = narrowPeakFiles, sep = '\t', col.names = peakscolNames)
  peakseqnames <- peak$chrom
  peakranges <- IRanges(start = peak$start, end = peak$end)
  peakstrand <- rep("*", length(peakranges))
  peaksymbol <- paste0(peak$chrom, ':', peak$start,'-', peak$end, seq(nrow(peak)))
  peak <- GRanges(seqnames = peakseqnames,
                   ranges = peakranges,
                   strand = peakstrand,
                   symbol = peaksymbol)
